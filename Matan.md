...
9. Докажите, что limx→0 sin(x)/x = 1.
...
8. Функция, непрерывная [в точке](#функция-непрерывная-в-точке) / [слева в точке](#непрерывность-слева-в-точке) / [справа в точке](#непрерывность-справа-в-точке) / [на интервале](#функция-непрерывная-на-интервале) / [на отрезке](#функция-непрерывная-на-отрезке)
9. Точка разрыва: устранимого / I-го рода / II-го рода [`[@]`](#точки-разрыва)
10. Асимптота; вертикальная / наклонная / горизонтальная асимптота
11. - Производная функции в точке [`[@]`](#производная-функции-в-точке)
    - Односторонняя производная в точке [`[@]`](#односторонняя-производная-в-точке)
12. Дифференцируемая функция [`[@]`](#дифференцируемая-функция)
13. Дифференциал первого порядка [`[@]`](#дифференциал-первого-порядка)
14. [Производная](#производная-n-го-порядка) / [дифференциал](#дифференциал-n-го-порядка) n-го порядка
15. - Невозрастающая / неубывающая / убывающая / возрастающая функция [`[@]`](#монотонные-функции)
    - Монотонная / строго монотонная функция [`[@]`](#монотонная-строго-монотонная-функция)
16. Строгий `1` / нестрогий `2` локальный минимум[`1`](null)[`2`](null) / максимум[`1`](null)[`2`](null) /
экстремум[`1`](null)[`2`](null)
17. Стационарная точка [`[@]`](#стационарная-точка)
18. Критическая точка [`[@]`](#критическая-точка)
19. Выпуклость вверх / вниз графика функции на промежутке [`[@]`](#выпуклость-вверх--вниз-графика-функции-на-промежутке)
20. Точка перегиба графика функции [`[@]`](#точка-перегиба-графика-функции)
...
> 0<|x−x_0|<δ ⇒ |ƒ(x)−a|<ε. Обозначение: $\lim\limits_{x→a}f(x)$=a, или $ƒ(x)→a$ при $x→x_0$.

#### Предел функции по Гейне

> Пусть функция f(x) определена в [проколотой окрестности](#проколотая-окрестность-точки) $\mathring{U}(x)$ точки $x_0$. Число $a$ называется пределом функции $ƒ(x)$ при $x→x_0$, если для любой последовательности {$x_n$} точек из $\mathring{U}(x_0)$, для которой $\lim\limits_{n→∞}x_n=x_0$, выполняется равенство $\lim\limits_{n→∞}f(x_n)=a$. Обозначение: $\lim\limits_{x→a}f(x)$=a, или $ƒ(x)→a$ при $x→x_0$.
...
> Последовательность {$x_n$} называется возрастающей, если $ƒ(n+1)>ƒ(n)$ $∀n$. Если последнее неравенство заменить на $f(n+1)<f(n), f(n+1)≥f(n)$ или $ƒ(n+1)≤ƒ(n)$, то получим определение соответственно убывающей, неубывающей и невозрастающей последовательностей.
...
> rar
Асимптота; вертикальная / наклонная / горизонтальная асимптота
...
> Функция $f : X → ℝ$ называется возрастающей, если $∀x_1,x_2∈X: x_1>x_2$ всегда верно $ƒ(x_1)>ƒ(x_2)$. Если последнее неравенство заменить на $f(x_1)<f(x_2), f(x_1)≥f(x_2)$ или $ƒ(x_1)≤ƒ(x_2)$, то получим определение соответственно убывающей, неубывающей и невозрастающей функций.
...
#### Строгий 1 / нестрогий 2 локальный минимум12 / максимум12 / экстремум12
