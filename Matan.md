...
Возьмём произвольный ε>0,5. Пусть число $\tau$ удовлетворяет условиям: $\tau>1$ (тогда $\frac ε t < ε$) и $\frac {|a|·|b|} {\tau} < \frac 1 2.$<br>
...
<br>$|f(x)g(x)-ab| = |ab±νa+μb±μν-ab| ≤ ν|a|+μ|b|+μν \lt \frac ε \tau|a| + \frac ε \tau|b| + (\frac ε \tau)^2 = ε( \frac {|a| + |b|} \tau + (\frac ε \tau)^2 ) \lt ε(\frac 1 2 + (\frac 1 2)^2) \lt ε.$ Значит, $\lim\limits_{x→x_0}f(x)g(x)=ab,$ ч.т.д.

#### Альтернативное доказательство по Гейне

Определения:

- [предел функции (по Гейне)](#предел-функции-по-гейне),
